...
import { prepareSubscriptionRefund } from "@/lib/server/subscription-cancel";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const blocked = protectMutation(request, { key: "subscription-refund", limit: 20, windowMs: 60_000 });
  ...
  return NextResponse.json({
    ok: true,
    result: prepareSubscriptionRefund(subscription)
  });
}
